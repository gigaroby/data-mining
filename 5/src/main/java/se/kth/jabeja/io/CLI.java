...
import se.kth.jabeja.config.AcceptanceFunction;
...
public class CLI {
  ...
  @Option(name = "-restartAfter", usage = "Restart simulated anealing after this number of rounds (-1 disables).")
  private int RESTART_AFTER = -1;
  ...
  @Option(name = "-acceptanceFunction", usage = "Acceptance function. Supported: SIMPLE, EXPONENTIAL")
  private String ACCEPTANCE_FUNCTION = "SIMPLE";
  ...
  public Config parseArgs(String[] args) throws FileNotFoundException {
    ...
    return new Config().setRandNeighborsSampleSize(randNeighborsSampleSize)
            ...
            .setAcceptanceFunction(ACCEPTANCE_FUNCTION)
            .setOutputDir(OUTPUT_DIR)
            .setAlpha(ALPHA)
            .setRestartAfter(RESTART_AFTER);
  }
}
